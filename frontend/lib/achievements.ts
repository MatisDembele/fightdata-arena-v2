...
export const ACHIEVEMENTS: Achievement[] = [
  ...
  { id: 'all_modes',        icon: '🧠', name: 'LAB MONSTER',      desc: 'Play all 7 quiz modes at least once.',        rarity: 'rare' },
...
export function checkAndUnlock(ctx: CheckContext): Achievement[] {
  ...
  const { mode, score, total, maxCombo, survived, slug, dailyScore, dailyRank, multiWon } = ctx
  ...
  // RARE — all modes
  const allModes = ['random', 'fighter', 'input', 'punish', 'hardcore', 'survival', 'damage']
  const played = allModes.filter(m => {
  ...
  if (played.length === allModes.length) u('all_modes')
