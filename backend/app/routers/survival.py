...
def submit_score(payload: ScorePayload, request: Request, db: Session = Depends(get_db)):
    ...
    if not (0 <= payload.best_score <= 9999):
        return {"ok": False}
    row = db.query(SurvivalScore).filter_by(player_name=name).first()
    if row:
        ...
        row = SurvivalScore(player_name=name, best_score=payload.best_score)
        db.add(row)
    db.commit()
    return {"ok": True}
...
def get_leaderboard(db: Session = Depends(get_db)):
    rows = (
        db.query(SurvivalScore)
        .order_by(SurvivalScore.best_score.desc())
        .limit(10)
        .all()
    )
    return [
